...
.config(function($routeProvider){
	$routeProvider.when('/', {
		...
		templateUrl : jcontext.getContextPath('/supervisor/module/category2/list.aon')
	}).otherwise({
		redirectTo : '/'
	});
});
category.controller('CategoryListCtl', function($cookies, $scope, $filter, $rootScope){
	...
	$scope.afterRoot = null;
	
	$scope.cateLoad = function(_id, event) {
		if($scope.afterRoot != null){
			$scope.afterRoot.removeClass('selected');
		}	
		$(event.target).addClass('selected');
		$scope.afterRoot =$(event.target);
		...
		var data = jcontext.loadJSON('/sys/category/listByrootId.json?rootId='+_id);
	...
	var init = function(){
		var cateData = jcontext.loadJSON('/sys/category/getListByParent.json?pid=0');
